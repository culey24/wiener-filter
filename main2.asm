...
#
# FIXED BY GEMINI:
# 1. Changed M_const from 2 to 10 (Matches test.asm)
# 2. Changed Correlation division from (N-k) to N (Matches test.asm logic)
# 3. Replaced explicit MSE calculation with MMSE Formula (Sigma_d^2 - Sum(w*Rdx))
#
# Assembler: MARS / SPIM
...
# =========================================================================
filename_input:   .asciiz "input/tc_0_input_5.txt"
...
after_y_compute:
    # ---------------------------------------------------------------------
    # SECTION 6: CALCULATE MMSE
    # Formula: MMSE = Variance(d) - Sum(w[k] * Rdx[k])
    # ---------------------------------------------------------------------
    
    # 1. Calculate Variance of Desired Signal (sigma_d^2)
    l.s $f20, const_zero    # sum_d_sq
    li $t0, 0               # n

calc_sigma_d_loop:
    bge $t0, $s5, finish_calc_sigma_d
...
finish_calc_sigma_d:
    mtc1 $s5, $f22          # N
    cvt.s.w $f22, $f22
    div.s $f16, $f20, $f22  # f16 = Initial MMSE (Variance)
    
    # 2. Subtract filter reduction term
    li $t0, 0               # k

calc_mmse_sum_loop:
    bge $t0, $s6, finish_mmse_sum_loop
    ...
    mul.s $f28, $f24, $f26
    sub.s $f16, $f16, $f28  # MMSE -= w[k] * Rdx[k]
    
    addi $t0, $t0, 1
    j calc_mmse_sum_loop

finish_mmse_sum_loop:
    # f16 now holds the final calculated MMSE
